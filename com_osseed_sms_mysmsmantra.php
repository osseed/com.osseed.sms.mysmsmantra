...
class com_osseed_sms_mysmsmantra extends CRM_SMS_Provider {
  ...
  /**
   * MysmsMantra API Server Session ID
   *
   * @var string
   */
  ...
  public $_apiURL = "http://bulksms.mysmsmantra.com:8080/WebSMS/SMSAPI.jsp";
   ...
   * Constructor
   *
   * Create and auth a MysmsMantra session.
   *
   * @return void
   */
  ...
  /**
   * Send an SMS Message via the MysmsMantra API Server
   *
  ...
  function send($recipients, $header, $message, $jobID = NULL) {
    if ($this->_apiType = 'http') {
      $postDataArray = array( );
      $query_string = '?username=' . urlencode($this->_providerInfo['username']) . '&password=' . urlencode($this->_providerInfo['password']) . '&sendername=' . urlencode($this->_providerInfo['api_params']['sender_id']) . '&mobileno=' . urlencode($header['To']) . '&message=' . urlencode(substr($message, 0, 160)); 
      $url = $this->formURLPostData($query_string, $postDataArray);
       ...
       * with MysmsMantra is assigned three queues namely 1, 2 and 3.
       */
     ...
     /**
      ...
      if ($isTest == 1) {
        $response = array('data' => 'Your message is successfully sent to:' . rand());
      }
      else {
      ...
      if ($send[0] == "Your message is successfully sent to") {
    ...
    if ($activity->find(TRUE)) {
      ...
      switch ($status) {
        case "001":
          ...
          $smsmantrastat = $this->_messageStatus[$status] . " - Message Unknown";
          break;

        case "002":
          $statusID = $actStatusIDs['Scheduled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Message Queued";
          break;

        case "003":
          $statusID = $actStatusIDs['Completed'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Delivered to Gateway";
          break;

        case "004":
          $statusID = $actStatusIDs['Completed'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Received by Recipient";
          break;

        case "005":
          $statusID = $actStatusIDs['Cancelled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Error with Message";
          break;

        case "006":
          $statusID = $actStatusIDs['Cancelled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - User cancelled message";
          break;

        case "007":
          $statusID = $actStatusIDs['Cancelled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Error delivering message";
          break;

        case "008":
          $statusID = $actStatusIDs['Completed'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Ok, Message Received by Gateway";
          break;

        case "009":
          $statusID = $actStatusIDs['Cancelled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Routing Error";
          break;

        case "010":
          $statusID = $actStatusIDs['Cancelled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Message Expired";
          break;

        case "011":
          $statusID = $actStatusIDs['Scheduled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Message Queued for Later";
          break;

        case "012":
          $statusID = $actStatusIDs['Cancelled'];
          $smsmantrastat = $this->_messageStatus[$status] . " - Out of Credit";
          break;
      }
      ...
      if ($statusID) {
        ...
        $activity->location = $smsmantrastat;
  ...
  function curl($url, $postData) {
    ...
    elseif ($response['http_code'] === 0) {
      return PEAR::raiseError('Cannot connect to the Mysmsmantra API Server.');
    }
